...
export class RadialLayout extends go.Layout {
  ...
  private _layerThickness: number = 100;
  ...
  public doLayout (coll: go.Diagram | go.Group | go.Iterable<go.Part>): void {
    ...
    while (it.next()) {
      ...
      if (layer === Infinity) {
        // Infinity used as init value (set in findDistances())
        continue
      }
      if (layer > maxlayer) {
        maxlayer = layer
  ...
  private radlay1 (vert: RadialVertex, layer: number, angle: number, sweep: number): void {
    if (layer > this.maxLayers) {
      // no need to position nodes outside of maxLayers
      return
    }
    // array of all RadialVertexes connected to 'vert' in layer 'layer'
    const verts: Array<RadialVertex> = []
    ...
    while (seen.count > 0) {
      ...
      least.edges.each(
        function (e) {
          ...
          if (finished.contains(neighbor as any)) {
            return
          }
